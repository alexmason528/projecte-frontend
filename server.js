...
app.get('/item/:type/:id', function(req, res) {
  ...
  fs.readFile(filePath, 'utf8', function(err, data) {
    ...
    request.get(`${API_BASE_URL}/api/item/${type}/${id}`, function(error, itemRes, itemBody) {
      const { name, details, images, estimations } = JSON.parse(itemRes.body)
      const sum = _.sumBy(estimations, 'value')

      const title = sum > 0 ? `${name} - Schätzung: ${numeral(sum).format('0,0')}€` : `${name} - Noch keine Schätzungen`
